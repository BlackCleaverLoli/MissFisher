# 更新日志
---
# Snapshot 24w37a　2024-09-10
> 7 个 Snapshot 的累积更新内容。  
>  
...
> 等待指定的优先竿型而无视最大竿时。适用如被迷津罗非鱼干扰的潜龙。
  ...
  在拍水目标参数或以小钓大标识符 `》` 后与竿时参数前插入被 `（）` 括号包围的单个竿型参数。
...
- 现在所有以小钓大阶段的竿时参数都支持写入完整的撒饵与非撒饵竿时。

  - 如 `！》~10+20！！` 。
- 现在如没有提供非撒饵竿时参数，将使用撒饵竿时参数除以 2 后的值作为录入的参数。
  - 如 `~10.3！` 将录入为最大撒饵竿时 `5.15` 秒、最大非撒饵竿时 `10.3` 秒
## BUG 修复及品质级改动
### 品质级改动
**停止命令**
- 现在可以省略默语前缀了。
...
- 修复了一个 BUG，该 BUG 曾导致：非海钓时大鱼模式的专一垂钓的使用条件判断错误。
---

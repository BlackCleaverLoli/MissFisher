# 更新日志
---
# Snapshot 24w37b　2024-09-12
> 8 个 Snapshot 的累积更新内容。  
>  
> 新增优先竿型参数与阶段参数；追加断杆日志适配；优化海钓。  
...
## 优先竿型参数
> 等待指定的优先竿型而无视最大竿时。适用如被迷津罗非鱼干扰的潜龙。
  
  在拍水目标参数后或以小钓大标识符 `》` 后与竿时参数前插入被 `（）` 括号包围的单个竿型参数。
- 将使空天姬在到达给定的最大竿时继续等待鱼咬饵而不执行断杆等操作，然后提起指定的优先竿型。

  - 测试：暂只接受单个竿型如 `（！！！）` 。
  - **[Wiki 条目](https://github.com/BlackCleaverLoli/MissFisher/wiki/Learn-Presets#16-%E4%BC%98%E5%85%88%E7%AB%BF%E5%9E%8B)** 。
## 阶段参数
> 现在空天姬将拥有在满足给定的目标数量时切换到其他预设的能力。该功能适用于一些拥有复杂钓法的鱼类，例如星鲸或红龙。

在所有手法参数与鱼识参数之间插入 `》阶段` 作为参数前缀标识，然后紧接想要在满足给定的目标数量后切换的预设，再使用 `《` 作为结束符。
...
- 现在所有以小钓大阶段的竿时参数都支持指定完整的撒饵与非撒饵竿时格式。
  ...
  - 如 `！》~10+20！！` 。
- 现在如只提供一个竿时，将使用此竿时减半后的值作为撒饵竿时，原来的值作为非撒饵竿时。

  - 如只提供 `~10.3` 将应用 `5.15` 为撒饵最长竿时、 `10.3` 为非撒饵最长竿时。
## BUG 修复及品质级改动
### 品质级改动
**停止命令**
- 现在可以省略默语前缀了。
...
- 【冲分模式】在幻海流清空采集力时，即使采集力不足 400 也将使用大鱼。
...
- 现在使用命令查看满足条件的内置预设时也将提示 “未经审核” 和 “低效预设” 。

- 低效预设的告警现在只要求写上 “低效预设” 而不再要求写上某符号。
- 现在内置预设的日期信息更清晰了。

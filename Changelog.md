# 更新日志
# MissFisher 1.11.14　2025-01-17
> 修复一个问题。
## BUG 修复及品质级改动
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当提供窗口期参数与阶段参数且预设内包含鱼识参数时，不会在前置阶段的窗口期前准备以小钓大用的鱼。
---
# MissFisher 1.11.13　2025-01-17
> 修复一个问题。
## BUG 修复及品质级改动
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当提供窗口期参数与阶段参数，且处于窗口期前时，不会开始准备以小钓大用的鱼。
---
# MissFisher 1.11.12　2025-01-17
> 修复一个问题。
## BUG 修复及品质级改动
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当退回到标题界面时，扩展功能的钓饵读取可能无法读取当前钓饵。
---
# MissFisher 1.11.11　2025-01-16
> 修复一个问题。
## BUG 修复及品质级改动
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：仅提供天气或 ET 窗口期参数之一并处于预拍水的预备时，即使当前距离切换天气要大于 3 分钟，有时也没有进行预拍水时的攒鱼计。
---
# MissFisher 1.11.10　2025-01-15
> 修复一个问题。
## BUG 修复及品质级改动
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当在距离窗口期不足 3 分钟时启动含有拍水鱼名参数的预设，预拍水的预备手法会错误地使用耐心 II 进行攒鱼计。
---
# MissFisher 1.11.9　2025-01-15
> 修复一个问题。
## BUG 修复及品质级改动
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当在窗口期外启动含有鱼眼手法参数的预设，并正常进入窗口期时，没有在窗口期内按预期地使用引诱和撒饵。
---
# MissFisher 1.11.8　2025-01-15
> 优化功能；修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 用户界面
- **主界面**  
现在预设输入框支持以格式 `预设名 分钟` 定时启动内置预设；  
如 `红龙 100` 可定时在 100 分钟后启动名为 `红龙` 的内置预设。
### 其他改动
- **命令列表**  
依照最新的功能修订了内容。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当在窗口期外启动自动功能时，没有在窗口期内按预期地使用引诱和撒饵。

- 修复了一个 BUG，该 BUG 曾导致：当经历过幻海流后使用内置预设，不会按预期地使用引诱和撒饵。
- 修复了一个 BUG，该 BUG 曾导致：设置定时的预设名过长时，定时悬浮窗的文本会意外地换行，并导致显示的内容不完整。
---
# MissFisher 1.11.7　2025-01-13
> 优化功能；修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 自动功能
- **耐心 II**  
当处于准备饵鱼的预备阶段时，即使提供了套娃手法参数，也将不使用。

- **引诱和撒饵**  
优化防止在使用这两个技能时使有效竿失效的措施；  
现在将检查以下窗口期的持续时间：  
给定的 ET 窗口期、给定的天气、幻海流、鱼眼和鱼识；撒饵将额外检查海域。  
判断时其一的剩余时间若少于 1 秒（撒饵是 3 秒），将不使用对应技能。

### 用户界面
- **航线预报**  
现在的尺寸将自适应。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：更换钓饵时没有按预期更换上可用的游动饵。

- 修复了一个 BUG，该 BUG 曾导致：没有播报更换钓饵时的尝试次数。
- 修复了一个 BUG，该 BUG 曾导致：当更换钓饵超时没有终止自动功能并告警。
- 修复了一个 BUG，该 BUG 曾导致：提供了天气参数并处于预拍水的预备时，即使当前距离切换天气要大于 3 分钟，有时也没有进行预拍水时的攒鱼计。
- 修复了一个 BUG，该 BUG 曾导致：有时鱼眼结束并再次进入循环时没有停止使用技能和抛竿。

---
# MissFisher 1.11.6　2025-01-11
> 修复一些问题。
## 功能更新
## 航线预报界面
- **新增航线预报界面**  
一个包含时间、航线标识描述、航线路线和调试用序号的航线预报界面。  
低占用地自动刷新并提供多达 36 条航线预报。
## BUG 修复及品质级改动
## 品质级改动
### 其他改动
- **程序性改动**  
修改触发计数模块检查的动作，以减少罕见情况下阶段未及时转换的情况。
---
# MissFisher 1.11.5　2025-01-10
> 修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 其他改动
- **程序性改动**  
从空天姬的源代码中清除了一批无用的动作属性。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：即使三语集合符中的鱼名没有错误，也会报告错误。
---
# MissFisher 1.11.4　2025-01-10
> 修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
...
### 自动海钓
- **拍击水面**  
当处于熟练渔技时，将有更大可能使用拍击水面。

### 其他改动
- **参数检查**  
现在因误写预设鱼名参数中三语集合符中的鱼名或 ID，导致展开后的正则表达式异常时（变为匹配空），将发送错误信息。

- **程序性改动**  
自动换饵的循环自我调用动作从发送日志改为触发器操作。  
提升个别条件中表达式的可读性。

- **提示性文本**  
修改了部分文本内容。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当从垂钓状态恢复到自由态，触发自动更换游动饵时，会发送多条更换成功的消息。
---
# MissFisher 1.11.3　2025-01-09
> 新增几个主题；修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 用户界面
...
### 自动海钓
- **触幻播报**  
当以内存读取的方式获取海域信息时，不再进行播报。
### 其他改动
- **预设水印**  
现在将随竿时悬浮窗一起自动隐藏（如已配置自动隐藏）。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：罕见情况下在参数切换后的自动换饵时，即使已经换上游动饵也没有继续自动动作。
- 修复了一个 BUG，该 BUG 曾导致：有时即使满足预期中的使用条件也没有使用撒饵。

- 修复了一个 BUG，该 BUG 曾导致：未处于自动海钓时，在海域开始后没有播报触幻钓法。
- 修复了一个 BUG，该 BUG 曾导致：`自动海钓` 当从内存获取海域时间轴，会错误判断当前航线的可用模式。
- 修复了一个 BUG，该 BUG 曾导致：`自动海钓` 当从内存获取海域时间轴，到达第 3 时段时会立刻结束海域。
- 修复了一个 BUG，该 BUG 曾导致：当同时提供鱼识参数与鱼眼手法参数，鱼眼的绝命竿结束的同时触发鱼识，没有切换到鱼识参数的预设。
---
# MissFisher 1.11.2　2025-01-05
> 修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 自动海钓
- **耐心**  
优化了耐心和耐心 II 在接近海域结束时的使用时机。
...
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：计数器被意外重置。
- 修复了一个 BUG，该 BUG 曾导致：获取的钓场名有误。
- 修复了一个 BUG，该 BUG 曾导致：提供鱼篓参数并处在预备饵鱼的手法时，错误地在采集力管理中计算了两次熟练妙招所需的采集力。
- 修复了一个 BUG，该 BUG 曾导致：`自动海钓` 幻海流时仅在蓝鱼模式拍水。

- 修复了一个 BUG，该 BUG 曾导致：错误地在采集力管理中计算了两次熟练妙招所需的采集力。
---
# MissFisher 1.11.1　2025-01-04
> 修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 启动播报
...
追加一条提示。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：拍击水面鱼名参数预期外地将目标竿时参数捕获为鱼名。

- 修复了一个 BUG，该 BUG 曾导致：`自动海钓` `成就模式` 幻海结束后会同时输入两条预设。
- 修复了一个 BUG，该 BUG 曾导致：同步时未授予仓库权限的说明文本中，启用权限的方法有误。
---
# MissFisher 1.11.0　2025-01-04
> 新增拍水切换预设；修复一些问题。
## 功能更新
## 拍水后预设参数
在所有前置手法参数及阶段预设参数后写 `》拍水后` 作为拍水后预设参数的标识，然后紧接想要在获得拍击水面效果后切换的预设。
  
- 在拍击水面效果失去时，将切换回原来的预设。
- 因参数位相同，与鱼识预设参数互相冲突。
- 当提供了阶段参数时，不会像鱼识参数那样在所有阶段内生效。
  - 但当存在阶段参数时，切换到了带有鱼识参数的预设后，鱼识参数仍会在所有阶段内生效。
* 同时在启动播报追加了相关文本。
## 多重提钩
> 将更智能地判断应该使用双提还是三提。

- 现在若存在鱼识参数或阶段参数，需要满足的数量条件小于指定的多重提钩时，将使用更少的提钩类型或单提。  
- 现在除海钓时的专一多提外，当采集力不足以使用三重提钩时，将尝试使用双重提钩。
## BUG 修复及品质级改动
## 品质级改动
### 自动功能
- **采集力管理**  
`自动海钓` 大鱼模式的最低保有「模式采集力」修改为 200（实际上这是个回调）。  
使用撒饵需要满足的采集力条件修改为与往期版本类似。
### 其他改动
- **计数**  
现在将在不同钓场抛竿时清空。

---
# MissFisher 1.10.9　2025-01-02
> 修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 内置预设
- **同步预设**  
触发同步动作时，若未授权远程仓库相关权限，将发出弹窗与默语消息提示。
### 其他改动
- **预设生成器**  
已移除入口（内置预设已补全所有除 7.0 普通稀有外的所有鱼类）。  
同时移除了「所有命令」中的相关命令。

## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：提供了收藏品参数并处于预拍水的攒鱼计时，可能预期外的同时使用耐心 II 和大鱼猎手。
---
# MissFisher 1.10.8　2025-01-01
> 修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 悬浮窗
...
现在切换到双采时不再保留悬浮窗。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：包含以小钓大参数时，即使提供了鱼篓参数的简写，也没有按预期将可存入鱼篓的鱼存入。
- 修复了一个 BUG，该 BUG 曾导致：提供了鱼篓参数并处于预拍水的攒鱼计时，可能预期外的额外使用耐心 II、熟练渔技、大鱼猎手。
- 修复了一个 BUG，该 BUG 曾导致：配置了延迟时间时，提供的拍击水面最大竿时若大于平钓最大竿时可能会误判目标竿并执行错误的行为。

- 修复了一个 BUG，该 BUG 曾导致：主界面没有在预期的日期应用新年主题。
---
# MissFisher 1.10.7　2024-12-31
> 优化体验。修复一些问题。新年快乐！
## 功能更新
## 预设语法
> 为识别「矛齿鲸」与「首领矛齿鲸」，E 端的首领矛齿鲸的差异部分在全名的后半，因此不能使用以往的支持的正则符号来排除。
- **计数目标参数**  
现在接受 `!` 号与 `()` 号。
## BUG 修复及品质级改动
### 自动功能
- **自动换饵**  
现在当换饵次数达 10 的倍数时将发送一条默语消息。
### 自动海钓
- **耐心**  
现在当携带海鸥风暴时将不使用耐心（仍会使用耐心 II）。
### 用户界面
- **主界面**     
现在启动时也会获取并尝试刷新赞助名单及预设说明。
### 其他改动
- **同步内置预设**  
现在同步成功时的消息将如以前那样包含内置预设说明。

- **程序性改动**  
现在海域的可抛竿时段开始与结束时将清除一次海鸥风暴与急速海豚的状态记录用变量。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：提供多提参数时专一垂钓预期外地没有使用。

- 修复了一个 BUG，该 BUG 曾导致：无法切换用户配置中的主界面主题复选框状态。
---
# MissFisher 1.10.6　2024-12-29
> 修复一个问题。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：未提供专一多重提钩参数时专一后会对任何竿使用歇竿。
---
# MissFisher 1.10.5　2024-12-29
> 修复一些问题。
## BUG 修复及品质级改动
### 预设参数
- **鱼名参数**  
现在支持输入转义符 `\` 以方便使用如 `\s` 来代替空格。
### 自动功能
- **专一垂钓**  
现在当所需的鱼识触发计数小于专一多提的最低数量时，将使用提钩而不是指定的专一后多重提钩。
### 其他改动
- **主界面**  
修改特殊主题中一个按钮的底部颜色。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：游动饵更换失败。

- 修复了一个 BUG，该 BUG 曾导致：`自动海钓` `成就模式` 当前不需要触幻时没有使用耐心 II 来攒鱼计。
- 修复了一个 BUG，该 BUG 曾导致：在自动功能暂停时打开主界面，中键启动按钮时会同时触发恢复与暂停。
---
# MissFisher 1.10.4　2024-12-27
> 新增一个惊喜。修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 悬浮窗
...
### 自动功能
- **采集力管理**  
提供平钓模式参数与收藏品手法参数、或提供了大鱼模式参数时，最低限度的采集力保证由 200 提升至 400 。 
...
钓饵读取方法现在将返回游动饵。  
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：`自动海钓` 断连没有起效。

- 修复了一个 BUG，该 BUG 曾导致：未提供钓饵参数时储存的当前钓饵变量为空。
---
# MissFisher 1.10.3　2024-12-27
> 修缮手法。修复一些问题。
## BUG 修复及品质级改动
## 品质级改动
### 自动功能
- **拍击水面**  
更新了防止丢失有效杆的措施：非海钓时，鱼识与鱼眼结束前 12 秒将不提起相应的拍击水面目标竿（海钓时是 8 秒）。
### 自动海钓
- **耐心II**  
现在预设中包含耐心模式参数时（例如耐心全提的预设），只要幻海的剩余时间大于 45 秒都会使用耐心 II。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：当处于耐心状态且携带专一垂钓时，会一直尝试使用提钩而不是相应的技巧提钩。

- 修复了一个 BUG，该 BUG 曾导致：即使未提供鱼识参数，也会限制撒饵与引诱的使用。
---
...
- **空天姬内存组件**  
新增回调方法可读取海钓相关信息。  
当因本体版本更新而初始化时，也将尝试注册。
## BUG 修复
- 修复了一个 BUG，该 BUG 曾导致：非海钓时即使条件满足也不使用专一垂钓。

- 修复了一个 BUG，该 BUG 曾导致：提供鱼影手法参数且未提供钓饵参数时进入窗口后未按预期开始钓鱼。

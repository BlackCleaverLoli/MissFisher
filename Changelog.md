# 更新日志
---
# Snapshot 24w38c　2024-09-17
> 12 个 Snapshot 的累积更新内容。  
>  
> 支持部分【金曦之遗辉】的技能与特性；新增【优先竿型】参数与【阶段】参数；优化海钓；优化功能。  

## 功能更新
## 金曦之遗辉
> 我们终于迎来了久违的捕鱼人技能改动，此次技改为捕鱼人新增了许多令人激动的新技能和新特性，但对游玩体验来说可能并不是什么好事……
- 适配 “收杆” 的日志行事件。
...
## 引诱*参数
> “M/A Lure” “豪引/细诱路亚” “雄心/谦虚诱饵”  —— 不管你怎么叫它，现在已以新的可接受参数类型把它们适配到空天姬。  

在技巧提钩参数后紧接 `引诱` 作为引诱参数标识符，使空天姬在 “抛竿” 后自动使用 “引诱” ；例如 `！！！引诱` 。
- 如提供了最大竿时区间参数，空天姬将先使用 2 次 “引诱” ，在到达最大竿时时使用第 3 次 “引诱” 而不 “断杆”。

- 如提供了技巧提钩参数，将使用与之对应的 “引诱” 技能类型。

- 与技巧提钩参数类似，空天姬也会自动判断要使用的 “引诱” 类型。
  - 只提供单个竿型时，使用与之对应的 “引诱” 类型。
  
  - 如果提供的多个竿型中包含 `！！！` 时，将默认使用对应 "强力提钩" 的 “引诱” 类型，否则必须指定技巧提钩参数。
- 该参数暂无 Wiki 条目。
...
- 预设例子请见 **[Wiki](https://github.com/BlackCleaverLoli/MissFisher/wiki/Learn-Presets#10-%E9%98%B6%E6%AE%B5)** 。
...
- 在幻海流带有大鱼且采集力将满时不再提钩防止溢出。
- 收窄了幻海流时清空采集力的剩余时间段：20 秒 → 18 秒
- 现在海钓结束时的最后一杆将尝试多提清空采集力。
- 启动时的断杆动作现在只会在未咬饵时时执行。
- 现在幻海流触发时将尝试解除拍击水面效果。
- 【冲分模式】在幻海流清空采集力时，采集力不足 400 时将使用大鱼。
- 【海钓模式】测试：索蒂斯的预设现在将使用阶段参数以提升鱼识触发效率。
...
**内置预设**
- 现在当首次切换到捕鱼人、或以捕鱼人职业更换区域时，将自动同步内置预设。

- 现在配置初始化时的内置预设同步将更加“静默”。
- 修改了预设预览的播报格式。

...
Phase1         ::= ExtraBiteOpt WS* BiteTimeOpt WS* BiteTypes WS* HooksetOpt  
                   ...
                   WS* InlineSpecialsOpt ;  
Phase          ::= ExtraBiteOpt WS* BiteTimeOpt WS* BiteTypes WS* HooksetOpt  
                   WS* InlineExtrasOpt WS* SwimbaitOpt WS* TargetsOpt  
                   WS* InlineExtrasOpt ;  
...
InlineSpecialsOpt ::= (WS* (InlineExtra | AtInlineSpecial))+ | ε ;  
InlineExtrasOpt   ::= (WS* InlineExtra)+ | ε ;  
AtInlineSpecial   ::= "@" WS* (InlineSlap | InlineExclusive) ;  
InlineExtra       ::= "||" WS* BiteTimeOpt WS* BiteTypes WS* HooksetOpt ;  
...
8) 内联特殊：  
   - 阶段顶层 `||` 可用于各阶段，必须显式给出咬饵类型，可选咬饵时间与提钩类型。  
   - 多个 `||` 按书写顺序保存，运行时在主目标之后、拍水之前从左到右短路判定。  
   - `||` 不带目标，不触发拍水预备、拍水目标采集力要求或 `拍水_SS` 状态检查。  
   - `||` 在括号、全局参数列表中仍表示列表分隔符；只有阶段体顶层按额外组解析。  
   - 内联拍水必须有目标（`<[...]` 或 `[...]`）。  
   - 内联专一必须有目标（`[...]`）。  
   - 若内联拍水未给咬饵类型，则使用阶段 1 的咬饵类型与咬饵时间。  
   - `@拍水` / `@专一` 仅允许阶段 1。  
   - 内联特殊允许出现在阶段目标之前或之后（用于兼容旧写法与序列化输出）。  
   - 序列化输出时会固定放在阶段目标之前，阶段 1 中顺序为 `||`、`@拍水`、`@专一`。  
   - 格式化输出时每条 `||` 会独立成行。  
   - 旧写法 `@拍水...【占位】` 会按显式 `||` 语义解析，并在序列化时输出为 `||...`。  

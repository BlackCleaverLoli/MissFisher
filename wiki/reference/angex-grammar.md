...
### 阶段内联特殊
`额外 | ex | 拍水 | 拍 | ss | 专一 | 专 | ic`  

补充：`@额外` 可用于各阶段；`@拍水` / `@专一` 仍仅用于阶段 1。  
...
InlineExtrasOpt   ::= (WS* "@" WS* InlineExtra)+ | ε ;  
InlineSpecial     ::= InlineExtra | InlineSlap | InlineExclusive ;  
InlineExtra       ::= ("额外" | "ex") WS* BiteTimeOpt WS* BiteTypes WS* HooksetOpt ;  
...
8) 内联特殊：  
   - `@额外` 可用于各阶段，必须显式给出咬饵类型，可选咬饵时间与提钩类型。  
   - 多个 `@额外` 按书写顺序保存，运行时在主目标之后、拍水之前从左到右短路判定。  
   - `@额外` 不带目标，不触发拍水预备、拍水目标采集力要求或 `拍水_SS` 状态检查。  
   - 内联拍水必须有目标（`<[...]` 或 `[...]`）。  
   - 内联专一必须有目标（`[...]`）。  
   - 若内联拍水未给咬饵类型，则使用阶段 1 的咬饵类型与咬饵时间。  
   - `@拍水` / `@专一` 仅允许阶段 1。  
   - 内联特殊允许出现在阶段目标之前或之后（用于兼容旧写法与序列化输出）。  
   - 序列化输出时会固定放在阶段目标之前，阶段 1 中顺序为 `@额外`、`@拍水`、`@专一`。  
   - 旧写法 `@拍水...【占位】` 会按显式 `@额外` 语义解析，并在序列化时输出为 `@额外...`。  

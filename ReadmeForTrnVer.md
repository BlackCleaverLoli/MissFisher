# 你正在查看 Triggernometry 版的使用说明
> [!TIP]  
> 要查看 Dalamud 版使用说明，请 [点这里](https://github.com/BlackCleaverLoli/MissFisher/blob/main/README.md) 。  
...
> `|` 分隔互斥选项。
>
> 如：`/e mfovl <竿时|time|采集力|gp> [水平偏移] [垂直偏移]`
> 下列命令都会被接受：
> `/e mfovl time`
...
`/e mfovl <竿时/time/采集力/gp> [水平偏移] [垂直偏移]`<br>`/e 空天姬悬浮窗 <竿时/time/采集力/gp> [水平偏移] [垂直偏移]`|修改悬浮窗坐标偏移 |偏移量支持负数；值越大，位置相对初始位置越靠近画面右下角

...
钓法表达式是一种专门设计给自动钓鱼插件 MissFisher（下称 MissFisher）的文本指令。它由一系列预定义的关键字、操作符与定界符，以及游戏内物品名称或 ID 等字面量组成，用于声明式描述特定场景下的钓鱼策略，从而快速配置并启动 MissFisher 的自动钓鱼功能。
...
- 这样写，MissFisher 将在准备状态使用抛竿，在任何类型的咬饵提示出现时使用提钩；
 并在以小钓大系技能可用时以小钓大，然后在以小钓大后对 `!!` 使用提钩；
  - 这里还使用了模式关键字 `大鱼`（`bf`），因此当以小钓大II冷却时，还会使用大鱼猎手来捕获大尺寸鱼类以获取以小钓大的机会。
```angex
大鱼》全部》！！
bf > ! > !!
```
...
在下一个 `》` 或 `>` 后的钓法参数中加入会导致以小钓大循环的鱼类的咬饵标记，以此处理包含以小钓大循环的以小钓大链。

- 这样写，MissFisher 将能自动处理以小钓大循环，同时也会在使用咬饵类型为 `!!` 的鱼类以小钓大后，在咬饵类型为 `!!!` 的提示出现时使用提钩。
  - 这里还使用了更适合处理以小钓大循环的模式关键字 `耐心`（`pt`）。
```angex
耐心》！！》！！+！！！
pt > !! > !! + !!!
```
...
```
### 钓法修饰关键字
可使用不同的钓法修饰关键字来直接控制 MissFisher 的钓鱼策略；钓法修饰关键字应写在 `=`、最终目标参数与计数器后。

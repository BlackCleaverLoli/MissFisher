...
例如，钓场「红茶川水系下游」的钓场之王「酿血鱼」（ID 7696），它在各种钓法攻略/工具中的钓法常这样表示：  
  
> 「蝲蛄丸子」→ `!!!`「酿血鱼」  
  
翻译一下就是：用「蝲蛄丸子」抛竿、就有可能有 `!!!` 类型的咬饵；对 `!!!` 提钩，就有可能钓到「酿血鱼」。  
...
在钓法表达式中，`大鱼` / `bf` 表示在钓法中需要使用大鱼猎手，以提高获得大尺寸鱼类的概率。  
  
**- 通常而言，我们在以下场景使用：**  
目标鱼类需要进行 1 次以小钓大后才会咬饵，且用来以小钓大的鱼的咬饵特征非常好分辨；  
又或者，目标鱼类是仅抛竿就能钓到的收藏品。  
  
例如，钓场「彻悟洞窟」的钓场之王「巨陨龟」（ID 15631），它在各种攻略/工具中的钓法常被这样表示：  
  
> 「金属亮片」→ `!`「花岗蟹」→ `!!!`「巨陨龟」  
  
翻译一下就是：用「金属亮片」抛竿、对 `!` 提钩，再用「花岗蟹」以小钓大、对 `!!!` 提钩，就有可能钓到「巨陨龟」。  
也就是说，「巨陨龟」只需要 1 次以小钓大。  
  
同时，在「彻悟洞窟」使用「金属亮片」时，咬饵的 `!` 必定是「花岗蟹」，咬饵的 `!!` 必定是「熔岩鲎」。  
...
- 先写模式关键字 `大鱼` / `bf`，然后阶段起始符，然后写需要提钩的用来以小钓大的鱼类的咬饵类型  
```angex  
大鱼》！  
bf > !  
```  
  
- 再接上第 2 个阶段起始符，表示在抛竿阶段结束后，尽可能地使用以小钓大  
```angex  
大鱼》！》  
bf > ! >  
```  
  
- 最后写上以小钓大后，需要提钩的目标鱼类的咬饵类型  
```angex  
大鱼》！》！！！  
bf > ! > !!!  
```  
表示钓法：尽可能使用大鱼猎手，在有多余采集力时使用撒饵；抛竿，对 `!` 提钩，尽可能使用以小钓大，在第 1 次以小钓大后、对 `!!!` 提钩。  

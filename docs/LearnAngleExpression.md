...
又或者，你可以在 MissFisher 的钓法[表达式编辑器](#02-钓法表达式编辑器)中启动。

# 0. 开始

## 0.1 环境
- FF14：已运行、Dalamud 已注入
- MissFisher：最新版、已经安装、启用且已认证
...
- 校验钓法表达式的语法是否合法
- 以更可读的形式将单行钓法表达式按表达式的结构来格式化呈现
  - 请注意，钓法表达式是单行的，在传播和在聊天窗口中使用时，应该复制为单行形式 

## 0.3 游戏条件
你的捕鱼人职业需要已经满级，且已经学习技能 耐心II。
...
nm > !
```
表示的钓法：使用 撒饵，然后 抛竿，对 `!` 类型的咬饵使用 提钩。

其中，`平钓` / `nm` 是以小钓大模式关键字；`》` / `>` 是阶段起始符；`！` / `!` 是阶段参数中的咬饵类型符。

## 2. 以小钓大模式
## 2.1 平钓
`平钓` / `nm` 会让 MissFisher 不使用提高大尺寸概率的技能，例如大鱼猎手或耐心系技能。

当目标鱼类不需要以小钓大就能钓到时，应在以小钓大模式使用关键字 `平钓` / `nm`。

**实测：**
- 钓饵：沙蚕
- 钓场：永夏岛北

手动装上钓饵 沙蚕，然后在钓法表达式编辑器中输入下面的钓法表达式：
```angex
平钓》！
nm > !
```
...
**启动后预期的行为：**
MissFisher 会使用撒饵，然后抛竿，然后对 `!` 类型的咬饵使用提钩。在回到自由态后，继续撒饵、抛竿……
...
- `平钓` / `nm` 
  - 通常而言，我们会在目标鱼类不需要 以小钓大 就能钓到时使用这个关键字

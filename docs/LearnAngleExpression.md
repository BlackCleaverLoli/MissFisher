...
简而言之：钓饵是「金属亮片」，抛竿后对 `!` 提钩，然后用「花岗蟹」以小钓大，对 `!!!` 提钩。
...
我们可以这样写包含 `大鱼` / `bf` 的用来钓「巨陨龟」的钓法表达式：  
  
- 先写「花岗蟹」的钓法，使用模式关键字 `大鱼` / `bf`，然后阶段起始符，然后写需要提钩的用来以小钓大的鱼类的咬饵类型  
```angex  
大鱼》！  
bf > !  
```  
...
简而言之：钓饵是「蛾蛹」，抛竿后对 `!` 提钩（如果处在耐心系技能的状态效果下，则使用精准提钩而不是提钩，下同），然后使用「斑纹虾虎」以小钓大，对 `!!!` 提钩（精准提钩）。

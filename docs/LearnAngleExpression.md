...
---  
# 5. 提钩类型
咬饵类型后的提钩类型关键字表示在对应的咬饵出现时应使用的提钩。
...
---  
# 6. 用来以小钓大的目标
以小钓大参数开始的 `》` / `>` 前的、被 `【 】` / `[ ]` 包裹的名称或数字 ID 表示用来以小钓大的目标鱼类。
...
- **表达钓法：**  
  直接抛竿、然后对任何咬饵提钩；  
  如果钓到了  `紫鱼` / `Calicia`，则对其使用拍击水面。

- 如果钓法需要以小钓大，拍水内联表达式会像这样：
```angex  
耐心》全部@拍水【紫鱼】》全部  
```  
```angex  
pt > all @ss [Calicia] > all  
```  
...
```  
- **表达钓法：**  
  ……如果钓到了  `36468` 或者 `36470`，则对其使用拍击水面。


## 4.1.1 拍击水面目标的咬饵参数
在 `拍水` / `ss` 后、拍击水面目标前、写额外的抛竿后目标咬饵参数

## 4.1.1 多个目标的写法
使用 `、` / `||` 来列举多个目标


@拍水！【山峰蟹|12718、山地鲀|12762】  
施工中……

...
> - **约定：**  
    在使用名称时，应同时使用数字 ID 来避免名称书写或识别时的错误  
...
- 例如「酿血鱼」，占据咬饵概率的 `4936` 的咬饵类型是 `!!`，不为拍击水面目标指定咬饵的话，就无法钓到 `4936` 并使用拍击水面。
```angex  
平钓》！！！@拍水【白妙鲤|4936】  
...
在模式关键字后用 `【 】` / `[ ]` 包裹钓饵名，来表示在这段钓法中要使用的钓饵。
...
## 预设表格
在[预设表格](https://docs.qq.com/sheet/DVlNaQ012amF4SW1G?tab=yirja0)，你可以找到实际场景下的实用钓法表达式作为例子来参考学习。

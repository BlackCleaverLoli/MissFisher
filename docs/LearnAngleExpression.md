...
在钓法表达式中，`平钓` / `nm` 表示在这段钓法中不需要任何提升鱼类大尺寸概率的技能。也就是说只需要抛竿，目标鱼类就有可能咬饵。  
...
在钓法表达式中，`大鱼` / `bf` 表示在钓法中需要使用大鱼猎手，来提高获得大尺寸鱼类的概率。  
...
我们可以这样写表达使用大鱼猎手的、用来钓钓场之王「巨陨龟」的钓法表达式：  
  
- 先写钓大尺寸的「花岗蟹」的表达式，使用模式关键字 `大鱼` / `bf`，然后阶段起始符，然后写需要提钩的用来以小钓大的鱼类的咬饵类型  
```angex  
大鱼》！  
bf > !  
```  
  
- 再接上第 2 个阶段起始符，表示在抛竿-提钩阶段结束后，尽可能使用以小钓大
```angex  
大鱼》！》  
bf > ! >  
```  
...
抛竿，对 `!` 提钩，尽可能使用以小钓大，在第 1 次以小钓大后、对 `!!!` 提钩。  
...
在钓法表达式中， `耐心` / `pt` 表示在钓法中需要使用「耐心系技能」（如耐心II），以在高频提钩场景下高效地提高获得大尺寸鱼类的概率。

**- 通常而言，我们在以下场景使用 `耐心` / `pt`：**  
目标鱼类 / 用来以小钓大的鱼类 需要大尺寸，咬饵频次高或特征难以分辨；
或是目标鱼类需要使用 2 次或以上次数的以小钓大才能钓到。

例如，钓场「叶脉水系」的钓场之王[「外科医生」](https://fish.ffmomola.com/#/wiki/fishing/spot/2/fish/7705)（ID 7705），它的钓法在各种攻略/工具中常被这样表示：

> 「蛾蛹」→ `!`（精准提钩）「斑纹虾虎」→ `!!!`（精准提钩）「外科医生」

简而言之：使用「蛾蛹」，抛竿后对 `!` 提钩（精准提钩），然后使用「斑纹虾虎」以小钓大，对 `!!!` 提钩（精准提钩）。

然而，在「叶脉水系」中，另一种鱼类拥有与「斑纹虾虎」一致的咬饵类型（都是 `!`），如果仍使用大鱼猎手来尝试钓用来以小钓大的鱼类的话，采集力会很快亏空

因此对于这种场景，我们使用耐心II而不是大鱼猎手。

我们可以这样写出表达需要使用耐心II的、用来钓钓场之王「外科医生」的钓法表达式：

- 先写包含 `耐心` / `pt` 模式关键字的、用来钓「斑纹虾虎」的表达式
  - MissFisher 会在角色持有耐心系技能的状态效果时，根据钓法表达式中的对应参数自动选用「精准提钩」或「强力提钩」 
```angex
耐心》！
pt > !
```

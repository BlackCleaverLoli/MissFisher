...
我们可以这样写表达使用大鱼猎手的、用来钓「巨陨龟」的钓法表达式：
...
- 最后写上以小钓大后，需要提钩的目标鱼类的咬饵类型，也就是对应「巨陨龟」的 3 个 `！` / `!`
```angex  
大鱼》！》！！！  
bf > ! > !!!  
```  
...
> 「残暴水蛭」→ `!`（精准提钩）「阿巴拉提亚公鱼」→ `!!!`（强力提钩）「一家之母」

简而言之：使用「残暴水蛭」，抛竿后对 `!` 提钩（精准提钩），然后使用「阿巴拉提亚公鱼」以小钓大，对 `!!!` 提钩（强力提钩）。
...
## 5.1 携带耐心状态时的提钩
咬饵类型后的 `强力` / `pw` 或 `精准` / `pc` 表示在携带耐心系技能的负面状态效果时，应对符合参数的咬饵使用的提钩技能。

- 例如，为 `!!!` 类型的咬饵显式表达应使用「精准提钩」
  - 省略时默认使用「强力提钩」
```angex  
耐心》！》！！！精准  
pt > ! > !!! pc  
```  
- **表达钓法：**  
  ……在以小钓大后对 `!!!` 提钩（如果携带耐心状态、则使用精准提钩）。

> MissFisher 会固定对 `!` 使用「精准提钩」、对 `!!` 使用「强力提钩」；对 `!!!` 默认使用「强力提钩」。

...
## 主要特性
- **内置手法**：空天姬内置优秀的资源管理与钓鱼手法，无需调教，即开即用。
...
> 否则，请参考 [安装指南](https://github.com/BlackCleaverLoli/MissFisher/blob/main/README.md#%E5%AE%89%E8%A3%85%E6%8C%87%E5%8D%97) 进行安装及配置。

## 主界面
空天姬拥有一个包含主要功能的用户界面，你可以通过以下方法来打开空天姬主界面：
- 在游戏中消息窗口的 `聊天输入栏` 输入命令 `/e ktj` - 按下 `回车键`。
  - 或者，右键触发器分组中的 `空天姬主界面` - 点击 `执行（强制/条件）` 。

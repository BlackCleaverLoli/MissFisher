...
> 顺位：在航线预报或 [FF14 鱼糕 航线标识](https://next.ffmomola.com/oceanFishing) 中从左到右的顺位，假设航线标识是 `气鲀四海¹/只有我最鳐摆²/冲分*` ，那么 `成就 2` 指 `只有我最鳐摆²`。
...
这些预设会被时不时被作者上传到 GitHub，你可以使用主界面的【同步按钮】或右键执行触发器 `更多` 分组下的 `在线同步内置预设` 来把它们同步到触发器中以供空天姬调用。

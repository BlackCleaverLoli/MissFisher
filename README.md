...
修改预设中的参数或钓法：[钓法表达式快速修改指南](https://blackcleaverloli.github.io/MissFisher/?page=QuicklyEditingExistingPresets)。  
...
> [!tip]  
> - **对于有窗口期限制的鱼类：**  
> 可在窗口期到来前启动， MissFisher 将自动准备相应技能资源和状态效果。  
> 除非备注中有特殊说明，建议的提前启动时间：50 分钟减去窗口期的持续时长，以避免钓点警惕。  
---  
...
*练级*|在出海垂钓中提升等级|如果你已完成 6.x 版本的主线任务<br>请尝试 [在宇宙探索练级](https://nga.178.com/read.php?tid=45353336)  
...
**准备出海垂钓：**  
1. 准备好至少 40 格或更多背包空间以及所需的钓饵  
必须携带的钓饵： `刺螠` `磷虾` `石沙蚕` 各 99 个；  
部分蓝鱼需要另外携带其他钓饵（详见航线预报）  
2. 点击 MissFisher 主界面的主页中的 `模式下拉菜单` 展开菜单 - 选择想要的模式。  
3. 在出海垂钓即将或已可登记时，前往 `利姆萨·罗敏萨` `X:3.0 Y:12.7` 与 NPC `德瑞斯库托塔` 对话 - 进行乘船登记。  
  
**在出海垂钓任务中启动：**  
1. 进入任务后 - 使角色站在甲板边缘并面朝船舷外  
2. 点击 `启动按钮`。  
...
> - **开始探索**  
> 如果你需要制作捕鱼人的宇宙探索工具，先从月面的探索任务开始，直到需要数据 V。  

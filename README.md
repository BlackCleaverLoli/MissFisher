...
2. 前往 `利姆萨·罗敏萨` `X:3.0 Y:12.7` 与 NPC `德瑞斯库托塔` 对话 - 进行乘船登记。  
...
> [!TIP]
> 使用【航线预报】功能可获取未来至多 36 班航线的信息与蓝鱼模式时所需钓饵的信息。
...
### 对于「云冠群岛」成就：
空天姬 MissFisher 有一些模式可帮助完成相关成就。
你可以点击主界面中的 `模式下拉菜单` 展开菜单选择想要的模式。

对应的空岛目标|空岛成就名|所需的钓饵
:-:|:-:|-
二期特供|新巢居住区的捕鱼人|云冠浮游虫
三期特供|落羽商业区的捕鱼人|云冠大蚊
四期特供|升歌文化区的捕鱼人|云冠大蚊、云冠气球虫
技巧点|天穹街的捕鱼人|云冠大蚊、云冠气球虫
特供绿鱼|观天的捕鱼人|万能拟饵、云冠大蚊、云冠气球虫、云冠红气球虫


> [!TIP]
> 进入云冠群岛，启动任意空岛模式，即可得到钓场位置情报。
>
> 建议的完成顺序：特供绿鱼 > 二期特供&三期特供 > 技巧点（四期特供）
...
> [!TIP]
> **可使用不同的主命令来使用 空天姬 MissFisher 命令**
> 如：`/e mf <子命令>` 等效于 `/e ktj <子命令>`。
> 
> **以下部分命令可不附带默语频道标识**  
> 如： `/e mf` → `/mf`；
> 需要在游戏中的「消息过滤」中启用「通知」-「错误信息」频道才能省略默语标识。 
## 打开窗口：
命令|描述
-|-
`/e mf`<br>`/e 空天姬`|打开主界面
...
`/e 空天姬鱼王`<br>`/e 空天姬鱼皇`<br>`/e mfBigFish`<br>`/e mfLegends`|以指定的钓场之王类型自动垂钓<br>（不限大小写）|
`/e 空天姬冲分`<br>`/e 空天姬蓝鱼`<br>`/e 空天姬成就1`<br>`/e 空天姬成就2`<br>`/e 空天姬练级`<br>`/e mfscore`<br>`/e mfscoring`<br>`/e mfbluefish`<br>`/e mfbf`<br>`/e mfachiev1`<br>`/e mfachiev2`<br>`/e mfleveling`<br>`/e mfxp`|以指定模式自动海钓|必须携带充足的「刺螠」「磷虾」「石沙蚕」（例如 99 个）<br>蓝鱼模式需要另外携带其他钓饵<br>留出 40 格或更多背包空间。<br><br>成就指需要在出海垂钓中钓到特定种类达到特定数量的鱼类才能获得称号的成就。<br>假设航线标识是「气鲀四海¹」「只有我最鳐摆²」<br>那么「成就2」指「只有我最鳐摆²」。
...
`/e 空天姬预设 查看 <鱼类全称>`<br>`/e mfpreset show <鱼类全称>`<br>`/e ktjys show <鱼类全称>`|查看对应的预设|以方便修改或作钓鱼准备
`/e mfdbg`|调试自动功能|
...
命令|描述|备注
-|-|-
`/e mflogtest`|测试关键日志行|按指引进行测试 需要已经开放副本：末日暗影亚马乌罗提

...
* **使用远程触发器下的自检工具箱进行自检**  
在 Triggernometry 找到 `远程触发器` 下的 `自检工具箱` - 阅读分组内的使用说明后逐项自检。
  
* **尝试重启游戏**  
关闭游戏然后重新打开，来将可能的重复的 鲶鱼精邮差 PostNamazu 注入从内存中清除。最后登陆角色，以触发空天姬换饵组件扩展方法的注册。
...
## 命令别名：
> 需要游戏中消息窗口可见系统错误信息（默认为红色字体），你需要勾选相应消息频道才能输入别名来使用功能  

...
4. 在主界面中点击 `配置` 按钮 - 在打开的用户配置界面中找到配置项 `强心剂键位` - 点击右侧的文本框 - 输入要使用的强心剂所处的热键栏上的热键位来设置 `强心剂键码` （详细说明见悬浮提示） - 根据自己的情况与喜好设置剩余的配置 - 点击 `保存` 以保存配置。
...
- 所有鱼王鱼皇（2.0 到 6.0）的预设均已补完。

鱼皇或需要施工前置的流程预设正在缓慢优化。

另外，还支持部分宠物、莫雯，这些预设来自作者与部分用户参与建设的在线表格 [空天姬内置预设表格](https://docs.qq.com/sheet/DVlNaQ012amF4SW1G?tab=BB08J2) 。

如果有缺少的鱼，你还可以通过 [内置预设反馈收集表](https://docs.qq.com/form/page/DVnBMaVNKQU1OVWlP) 来向作者请愿添加鱼类预设。
